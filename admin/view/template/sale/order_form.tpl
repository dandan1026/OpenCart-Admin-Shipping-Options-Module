    ...
    <div style="display: inline-block; width: 100%;">
      ...
      <div id="tab_order" class="vtabs_page">
        <table class="form">
          ...
          <tr>
            ...
            <td class="shipping_method"><?php echo $shipping_method; ?><input type="hidden" name="shipping_method" id="shipping_method" value="<?php echo $shipping_method; ?>"></td>
          </tr>
          <?php } ?>
          <tr>
          ...
          <tr>
            ...
            <td><?php echo $transaction_id; ?><input type="hidden" name="transaction_id" id="transaction_id" value="<?php echo $transaction_id; ?>"><input type="hidden" name="grand_total" id="grand_total" value="<?php echo $total; ?>"></td>       
          </tr>

          <tr>
            <td><?php echo $entry_date_processed; ?></td>
            <td>
                <span id="date_processed_holder">
                <?php if (!(is_null($date_processed))) { ?>
                    <?php echo $date_processed; ?>
                <?php } else { ?>
                   <button id="processButton" onClick="processOrder();">Process Order</button>
                <?php } ?>
                </span>
            </td>
          </tr>
      ...
      <div id="tab_product" class="vtabs_page">
        ...
        <table class="list">
          <thead>
            <tr>
              <td class="left" colspan="3">Shipping Options</td>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td class="left">Change Shipping</td>
			        <td class="left" colspan="2">
      			    <select id="shippingOptions" name="shippingOptions" style="width: 450px;">
      			    </select>
      			    <button id="updateShippingButton" onClick="updateShipping();">Update Shipping</button>
			  </td>
			</tr>
           
			<tr>
			  <td colspan="3><span class="add" onclick="addProduct();">&nbsp;</span></td>
			</tr>
...
function removeProduct(id) {
  ...
  selectedShippingOptionValue = $('#shippingOptions option:selected').val();

	$.ajax({
		url: 'index.php?route=sale/order/removeProduct&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>&order_product_id=' + id + '&shipping=' + selectedShippingOptionValue,
		dataType: 'json',
		beforeSend: function() {
			$('.success, .warning').remove();
		...
		success: function(data) {
			...
			//update field with new grand total
      $('#grand_total').val(data.product_data['formatted_grand_total']);
...
function addProduct() {
  ...
  selectedShippingOptionValue = $('#shippingOptions option:selected').val();
	...
	$.ajax({
		...
		dataType: 'json',
		data: 'shipping=' + selectedShippingOptionValue + '&product_id=' + encodeURIComponent($('#products').val()) + '&option=' + options + '&quantity=' + encodeURIComponent($('input[name=\'add_quantity\']').val()) /*+ '&tax=' + encodeURIComponent($('input[name=\'add_tax\']').val()) // TU */,
		beforeSend: function() {
			$('.success, .warning').remove();
			$('#product').before('<div class="attention"><img src="view/image/loading_1.gif" alt="" /> <?php echo $text_wait; ?></div>');
		},
...
function processOrder(){
  var r=confirm("Are you sure you are ready to process this order? The credit card will be charged.");
  if (r==true){
    $('#processButton').attr('disabled', 'disabled');
  	
    $.ajax({
  		url: 'index.php?route=sale/order/process&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>&transaction_id=' + $('#transaction_id').attr('value') + '&grand_total='  + $('#grand_total').attr('value'),
  		dataType: 'json',
  		beforeSend: function() {
  			$('#loading').remove();
  			$('#processButton').after('&nbsp;<img id="loading" src="view/image/loading_1.gif" alt="" />');
  		},
  		
		success: function(data) {
			if (data.error) {
				alert(data.error);	
				$('#processButton').attr('disabled', '');
			}
		  ...
		  //console.log(data.success);
			if (data.success) {
			 $('#date_processed_holder').html(data.success);
			}
		}
  	});
...
function getShippingOptions() {
	$('#shippingOptions option').remove();

	$.ajax({
		url: 'index.php?route=sale/order/shipping&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>',
		dataType: 'json',
		beforeSend: function() {
			$('#loading').remove();
			$('#shippingOptions').after('&nbsp;<img id="loading" src="view/image/loading_1.gif" alt="" />');
		},
		success: function(data) {
			$('#loading').remove();
			quoteData = data.fedex_wsdl.quote;
			//console.log(quoteData);
			
			//cost/text/title
      jQuery.each(quoteData, function(fedex_data, val) {
        //console.log(data.fedex_wsdl.quote[fedex_data]);
        cost = data.fedex_wsdl.quote[fedex_data].cost;
        text = data.fedex_wsdl.quote[fedex_data].text;
        title = data.fedex_wsdl.quote[fedex_data].title;
        
        if(title == $('#shipping_method').val()){
          $('#shippingOptions').append('<option value="' + cost + '" selected>' + title + ' - ' + text + '</option>'); 
        } else {
          $('#shippingOptions').append('<option value="' + cost + '">' + title + ' - ' + text + '</option>');        
        }
      });
...
//update shipping options after updating products
function updateShipping(){

  selectedShippingOptionValue = $('#shippingOptions option:selected').val();
  temp = $('#shippingOptions option:selected').text();
  temp = temp.split('-');
  selectedShippingOptionValueText = $.trim(temp[0]);
  
  	$.ajax({
  		url: 'index.php?route=sale/order/updateShipping&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>&shipping=' + selectedShippingOptionValue + '&shippingText=' + selectedShippingOptionValueText,
  		dataType: 'json',
  		beforeSend: function() {
  			$('.success, .warning').remove();
  			$('#updateShippingButton').after('<div class="attention"><img src="view/image/loading_1.gif" alt="" /> <?php echo $text_wait; ?></div>');
  		},
  		complete: function() {
  			$('.attention').remove();
  		},
  		...
  		success: function(data) {
  		  ...
  		  //return false;
				...
				$('.grand_total').html(data['newTotal']);
				//update field with new grand total
        $('#grand_total').val(data['newTotal']);
       
				$('.shipping_method').html(data['shippingText']);
				//update field with new grand total
        $('#shipping_method').val(data['shippingText']);        
        

				// TU START
				$('.taxes').remove();
			
				$('#totals .grand_total').parent().before('<tr class="taxes"><td></td><td colspan="4" class="right">Shipping & Handling:</td><td class="right">' + data['shipping'] + '</td></tr>');
				// TU END



				$('#tab_product #product').before('<div class="success">Shipping Updated</div>');
